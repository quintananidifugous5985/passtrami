...
const fixture = (USR: string, sites: string[], PWD = "fixture-secret") => ({ USR, sites, PWD });

Deno.test("builds the native website and username request", () => {
...
Deno.test("builds the native account-list request without requesting passwords", () => {
...
Deno.test("account lists filter domains, deduplicate usernames, and exclude password fields", () => {
...
Deno.test("account lists support numbered entries and an empty result", () => {
...
Deno.test("account lists preserve locked and native errors", () => {
  throwsCode(() => usernamesFrom({ STATUS: 9 }, "example.test"), "locked");
  throwsCode(() => usernamesFrom({ STATUS: 1 }, "example.test"), "native_error");
});
Deno.test("normalizes website URLs and rejects non-web schemes or embedded credentials", () => {
...
Deno.test("accepts the exact username and a true website subdomain", () => {
...
Deno.test("rejects a username prefix and lookalike website domains", () => {
...
Deno.test("does not return a password placeholder", () => {
  throwsCode(() => credentialsFrom({ STATUS: 0, Entries: [fixture("p", ["google.com"], "Not Included")] }, "google.com", "p"), "not_found");
});
Deno.test("handles native numbered entries and folds identical password values", () => {
...
Deno.test("keeps different matching passwords ambiguous for raw output", () => {
...
Deno.test("preserves native locked and no-result states", () => {
