    ...
    static func parse(_ arguments: [String]) throws -> Command {
        ...
        if arguments.first == "list" {
            guard arguments.count == 2,
                ...
                throw CLIError("Usage: passtrami list <domain>", exitCode: 64)
            }
        ...
        if arguments.first == "get" {
            guard arguments.count == 3,
                ...
                throw CLIError("Usage: passtrami get <domain> <username>", exitCode: 64)
            }
            return .get(domain: arguments[1], username: arguments[2])
        }
        throw CLIError("Usage: passtrami get <domain> <username> | list <domain> | status | --help", exitCode: 64)
    }
...
private let help = """
Usage:
  passtrami get <domain> <username>
  passtrami list <domain>
  passtrami status
  passtrami --help

Passtrami opens when needed and waits for unlock.
...
private func commandOutput(_ command: Command, response: [String: Any]) throws -> Data {
    guard response["ok"] as? Bool == true else {
        throw CLIError(response["message"] as? String ?? "The Passtrami request failed.")
    }
    switch command {
    case .help: return Data(help.utf8)
    case .status:
        guard let state = response["state"] as? String else {
            throw CLIError("Passtrami's response has no state.")
        }
        return Data((state + "\n").utf8)
    case .get:
        guard let password = response["password"] as? String else {
            throw CLIError("Passtrami's response has no password.")
        }
        return Data(password.utf8)
    case .list:
        guard let usernames = response["usernames"] as? [String] else {
            throw CLIError("Passtrami's response has no account list.")
        }
        guard usernames.allSatisfy({ !$0.contains(where: { $0.isNewline }) }) else {
            throw CLIError("Passtrami's response has an invalid username.")
        }
...
private func run() throws {
    ...
    guard let response = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
        ...
        throw CLIError("Passtrami sent an invalid response.")
    }
...
do {
    ...
    try? writeOutput(Data(("passtrami: " + failure.message + "\n").utf8), to: STDERR_FILENO)
