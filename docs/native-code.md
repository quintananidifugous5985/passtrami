...
| `MCP/PasstramiMCP.swift` → `MCP/CredentialServer.swift` | Serve stdio MCP tools and agent instructions. Return metadata only. |
...
`Passtrami.xcodeproj` is the build definition. Its **Passtrami** scheme builds the app plus **PasstramiCLI**, **PasstramiEngine**, and **PasstramiMCP**, and embeds the helpers in the app. MCP uses the official Swift SDK through Swift Package Manager. Source folders stay synchronized with disk; there is no project generation step.
